...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  try {
    ...
    return Response.json({ item: await fusionRepository.read((store) => store.getAsset(id)) });
  } catch (error) {
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  try {
    ...
    return Response.json({ item: await fusionRepository.mutate((store) => store.updateAsset(id, body)) });
  } catch (error) {
    return apiError(error);
  }
}
...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  try {
    const { id } = await params;
    await fusionRepository.mutate((store) => store.deleteAsset(id));
